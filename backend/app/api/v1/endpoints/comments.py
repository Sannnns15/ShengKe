...
from app.schemas.comment import CreateCommentRequest, CommentResponse, CommentTreeItem
...
@router.get("/moments/{moment_id}/comments", response_model=Result[list[CommentTreeItem]])
async def list_moment_comments_endpoint(
    moment_id: UUID,
    db: AsyncSession = Depends(get_db),
    user_id: UUID = Depends(get_current_user_id),
):
    """Get tree-structured comments for a Moment with author info."""
    # Verify moment exists
    moment = await get_moment(db, moment_id, user_id)
    if moment is None:
        return Result(code=1404, message="时刻不存在或无权访问", data=None)

    tree = await get_moment_comments(db, moment_id)
    items = [CommentTreeItem(**c) for c in tree]
    return Result(
        code=0,
        message="success",
        data=items,
...
async def delete_comment_endpoint(
    ...
    """Soft-delete a comment (author only). Replaces content with placeholder."""
